...
class FunSetSuite extends FunSuite {
  ...
  test("diff contains s-t elements") {
    new TestSets {
      val u1 = union(s1, s2)
      val u2 = union(s1, s3)
      val i = diff(u1, u2)
  ...
  test("filter contains s for p holds") {
    new TestSets {
      val u1 = union(s1, s2)
      val f = filter(u1, x => x == 1)
  ...
  test("forall") {
    new TestSets {
      val u1 = union(s1, s2)
      val u2 = union(s1, s3)
      val b1 = forall(u1, x => x < 3)
      val b2 = forall(u2, x => x < 3)
      assert(b1, "Forall 1")
      assert(!b2, "Forall 2")
    }
  }
  ...
  test("exists") {
    new TestSets {
      val u1 = union(s1, s2)
      val u2 = union(s1, s3)
      val b1 = exists(u1, x => x == 2)
      val b2 = exists(u2, x => x == 2)
      assert(b1, "exists 1")
      assert(!b2, "exists 2")
    }
  }
  ...
  test("map") {
    new TestSets {
      ...
      val u1 = map(union(s1, s2), inc)

      assert(!contains(u1, 1), "Map 1")
      assert(contains(u1, 2), "Map 2")
      assert(contains(u1, 3), "Map 3")
    }
  }
}
